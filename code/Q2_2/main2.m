...
mid = round(0.5*size(X,1));
X1 = X(1:mid, :);
X2 = X(mid+1:size(X,1), :);
Y1 = Y(1:mid, :);
Y2 = Y(mid+1:size(X,1), :);

div = round(0.8*size(X1,1));

X_train1 = X1(1:div,:);
Y_train1 = Y1(1:div,:);
X_val1 = X1(div+1:size(X1,1),:);
Y_val1 = Y1(div+1:size(Y1,1),:);
...
[w11, w21, w31, w41, v1, train_error1, val_error1] = MLP_Train_Final(X_train1, Y_train1, X_val1, Y_val1, nEpochs, eta, mini_Bsize, d_Per);
save('weights1.mat', 'w11', 'w21', 'w31', 'w41', 'v1');
